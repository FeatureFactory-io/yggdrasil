...
APPEND TO GUIDANCE (Step 6, Implementation Steps, Section 3 "Testing"):

---

## TAF Integration: Test Planning by Level

When planning tests for a feature, explicitly plan all four levels from the Test Trophy:

1. **Unit tests** (thin): Only for custom QuerySet methods, pure utility functions, or isolated business logic. Do NOT plan unit tests for default Model CRUD or auto-generated views.

2. **Integration tests** (thick — main bet): Plan for every service method, every CRUD path, every validation rule. NO MOCKING. Use real objects, real DB, real connections.

3. **Acceptance tests** (behave): Plan which .feature scenarios map to this feature. Check the TAF Step Library Catalog — assemble scenarios from existing steps. If steps are missing, note that TAF-07 must be invoked.

4. **E2E tests** (selective): Only plan E2E if this feature is part of a critical cross-feature user journey.

Reference: Test Architecture Document (produced by TAF-01).

## Inputs (additional)

- **Test Architecture Document** (Document, Required) — produced by TAF-01 Define Test Architecture.
