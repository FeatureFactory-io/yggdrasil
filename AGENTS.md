...
1. **PRD:** `PRD.MD` — product mission, personas, use cases
2. **Architecture:** `docs/architecture/SAO.md` — all technical decisions
...
## Review gates (hard stops)

Human gates in `.cursor/plans/yggdrasil_inception_bootstrap_52e9f8ae.plan.md` are **blocking checkpoints**, not auto-completable todos.

1. **Never mark a review gate complete** unless the user explicitly approves in chat (e.g. approve, approved, LGTM, sign-off).
