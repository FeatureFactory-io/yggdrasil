...
**Views v2 (Content + viewport, W15):** Extends `BrowseView.payload` with nullable `content.field_map` (stereotype slug → ordered field paths for graph labels, edge labels, and table columns) and `viewport` (graph-only `{ zoom, pan, center_element_id }`, opt-in on save). Live URL encodes visible fields via repeated **`field_{stereotype}={path}`** params (mockup-validated); **`?content=` preset URLs are superseded**. `graph.json` accepts resolved field_map and returns multiline **`Key: value`** node labels. v1 records without these keys default to name-only labels. See [`VIEW-BROWSE-1_VIEWS_V2_MOCKUP_RECONCILIATION.md`](../plans/act-2-view-browser/VIEW-BROWSE-1_VIEWS_V2_MOCKUP_RECONCILIATION.md) and [`VIEW-BROWSE-1_VIEWS_V2_CONTENT_VIEWPORT_CR.md`](../plans/act-2-view-browser/VIEW-BROWSE-1_VIEWS_V2_CONTENT_VIEWPORT_CR.md).
